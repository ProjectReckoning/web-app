...
export async function get5LastTransactionsUsecase(pocketId?: string): Promise<TransactionSummaryEntity[]> {
  const result = await transactionHistoryRepository.getLast5Transaction(pocketId);
  return result;
}